...
use utf8;
#use encoding 'UTF-8';
require("PD_common_funcTEST2.cgi");
...
my $selfPage = "http://$homeIP/cgi-bin/Operations/healthmonitorTEST.cgi";
   my $Fname;
   my $Lname;
   my $user_dept;
   my $user_email;
...
#PRINT PRODUCTION
	...
	my @genqueues = ("GENERIC_ELEC_EXT|RIPE_PRT_EXT","BACKEND_ELEC","XYWRITE","PACKAGE|PACKAGE_SINGLE","TURBOX6_GENERIC","TURBOX2_GENERIC|TURBOX_GENERIC|TURBOXQ_GENERIC|TURBOX6ADGEN_GENERIC|TURBOX2STV_GENERIC","VERIFY_BECKY|VERIFY_NORM");
	my @ripeSystems = ("ripe01|ripe02","ripe03|ripe04","ripe05|ripe06","ripe07|ripe08|ripe09");
	my @cronacleSystems = ("qbrwagentprod01","qbrwagentprod02");
	...
	my @Cronacle = ("qbrwagentprod01","qbrwagentprod02");
#if ($prodType eq "PRINT") {
if (($prodType eq "PRINT") || ($prodType eq ""))  {
	...
	if ($pageMode eq  "ELEC") {
		$sth = $dbh->prepare("SELECT JOBID as JOBID, CUSTCODE as CLIENTID, PRODUCTLOOKUP as ProductName, SYSTEM as SYSTEM, STARTTIME as STARTTIME, JOBCHAIN as JOBCHAIN from TVSCHEDULES WHERE SCHEDDATE = CURDATE() AND JOBCHAIN = \"SCHEDULE_RIPE_BUILD\"");
	} elsif ($pageMode eq  "ADGENPRINT") {
		$sth = $dbh->prepare("SELECT JobId as JOBID, CustomerName as CLIENTID, ProductName as ProductName, System as SYSTEM, StartTime as STARTTIME, JobChain as JOBCHAIN from MOVIESCHEDULES WHERE SCHEDDATE = CURDATE() AND JOBCHAIN = \"ELECTRONIC_ADGEN_CHAIN\"");
	} elsif ($pageMode eq  "ADGENELEC") {
		$sth = $dbh->prepare("SELECT JobId as JOBID, CustomerName as CLIENTID, ProductName as ProductName, System as SYSTEM, StartTime as STARTTIME, JobChain as JOBCHAIN from MOVIESCHEDULES WHERE SCHEDDATE = CURDATE() AND JOBCHAIN = \"ELECTRONIC_CHAIN\"");
	} else {
...
foreach my $thisOne (@genqueues) {
	...
	EndDIV();
		...
		if ($returnCount == 0) {
			...
			while ( my @rows =  $sth2->fetchrow_array() ) {
				...
				my @fieldValsGen = $sth2->fetchrow_array();
				my %fieldContentG;
			...
			## get all jobs in batch queue
				...
				#	print "&#09;&#09;$fieldContentJob{\"NAME\"} - $fieldContentJob{\"STATUS\"}\n<br>";
					...
					if ($thisBatch =~ /^VERIFY/) {
						...
						if (($DivLate ne "") && (($prodType eq "ELECTRONICGLEN") && ($custMaster[0] eq "CCE"))){
							#ClosedDIV("class","jobitemVerify$DivLate","$fieldContentJob{\"PARAM2\"} <br>CID $CID Due at $dueTime");
							$thisAV = "$CID|$dueTime|$fieldContentJob{\"PARAM2\"}|$DivLate";
							@av_list = (@av_list, $thisAV);
						}
	...
	if (($prodType eq "PRINT") || ($prodType eq "ELECTRONICGLEN")) {
		...
		my $allcidtime = "";
			foreach (@avsort) {
				...
				if (($newCID eq $oldCID) && ($newTime eq $oldTime)) {
					...
					ClosedDIV("class","jobitemVerify$avParts[3]","<center>CID $newCID Due at $newTime</center>");
					ClosedDIV("class","jobitemVerifyplain","<center>$newFile</center>");
				}
				$oldCID = $newCID;
				$oldTime = $newTime;
	...
	print "</a>";
	my @sums = getAlphaQTotals(@ripeSystems);
	if ($sums[5] > 0) {
	ClosedDIV("class","FlexSidebarSumBAD","Late2Client &nbsp;&rarr;&nbsp;$sums[5]");
	}
	if ($sums[1] > 0) {
	...
	print "</a>";
	my @sums = getAlphaQTotals(@cronacleSystems);
	if ($sums[5] > 0) {
	ClosedDIV("class","FlexSidebarSumBAD","Late2Client &nbsp;&rarr;&nbsp;$sums[5]");
	}
	if ($sums[1] > 0) {
			...
			foreach my $thisBatch (@findBQueues) {
				...
				while ( my @rows =  $sth->fetchrow_array() ) {
					...
					if ($thisBatch =~ /^VERIFY/) {
						...
						if (($DivLate ne "") && ($custMaster[0] eq "CCE")){
							$late2clientCCE++;
						}
...
return($pending,$retained,$executing,$stopped,$late2clientCCN,$late2clientCCE);
} #sub getAlphaQGroup {
